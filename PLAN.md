...
- [x] 音階教學頁（指板音階高亮、級數/音名切換、把位 Box、點音試聽、播放整條音階）
- [x] 節奏引擎（取代原節拍器規劃）：8/16 Beat 刷弦、琶音、合成鼓組 backbeat、BPM 可調
- [x] 歌曲/進行資料庫 + 播放跟弦動畫（13 組進行：日系五大＋歐美經典，小節高亮）
- [x] 日系進行包（王道/小室/卡農/丸サ/4536251）＋ 12 調移調器
- [x] 和弦樂理教學（音程結構、和弦變化教室）與樂理練習測驗頁
- [x] 和弦代號擴充（add9/6/m6/dim/m7♭5/斜線和弦）＋調性字典頁（順階和弦表）＋級數練習題
- [x] 色彩和弦切換（原版/七和弦/add9）、借用和弦進行包（IVm/♭VI–♭VII/副屬鏈/♯IVm7♭5/低音下行）、転調模式
