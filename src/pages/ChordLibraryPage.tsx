  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
          {visibleChords.map((chord) => (
            ...
            >
              <span className="mb-1 text-lg font-bold text-ink-900">
                {chord.chordName}
              </span>
              <span className="mb-2 text-xs text-ink-600">
        ...
        {visibleChords.length === 0 && (
          <p className="mt-8 text-center text-ink-600">此分類目前沒有和弦。</p>
        )}
      ...
      <aside className="w-full shrink-0 lg:w-80">
        <div className="sticky top-6 rounded-2xl border border-line-200 bg-paper-100 p-6">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {formula.intervals.map((s, i) => (
                <span
                  key={s}
                  className="flex flex-col items-center rounded-md bg-paper-300 px-2 py-1"
                >
          ...
          <div className="mb-4 space-y-2 rounded-lg bg-paper-300/60 p-3 text-xs leading-relaxed text-ink-600">
            <p>
              <span className="mr-1 font-semibold text-navy-700">怎麼變化</span>
              {formula.changeText}
            </p>
            <p>
              <span className="mr-1 font-semibold text-gold-700">什麼時候用</span>
              {formula.whenToUse}
            </p>
          </div>

          {selected.note && (
            <p className="mb-4 rounded-lg bg-paper-300/60 p-3 text-sm leading-relaxed text-ink-700">
              {selected.note}
            </p>
          )}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 rounded-lg bg-navy-700 py-2 font-semibold text-white transition-colors hover:bg-navy-600"
            >
              ♪ 刷弦
            </button>
            <button
              onClick={() => playChord(selected, "arpeggio")}
              className="flex-1 rounded-lg bg-paper-400 py-2 font-semibold text-ink-900 transition-colors hover:bg-paper-400"
            >
              ♪ 分解
            </button>
          </div>
